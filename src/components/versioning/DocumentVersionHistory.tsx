...
const DocumentVersionHistory: React.FC<DocumentVersionHistoryProps> = ({ document }) => {
            ...
            {allVersions.map((version, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {selectedVersion?.id === version.id && compareVersion === null && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleRestore();
                ...
                <p className="text-sm font-medium text-gray-900 mt-2">
                  {version.id === 'current' ? 'Current Version' : `Version ${version.number}`}
                </p>
                ...
                {selectedVersion?.id === version.id && compareVersion === null && version.id !== 'current' && (
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const currentVersionIndex = allVersions.findIndex(v => v.id === 'current');
          ...
          {!selectedVersion ? (
            ...
            <div>
              ...
              <div className="flex items-center justify-between mb-4 bg-gray-50 p-3 rounded-md">
                <div>
                  <span className="text-sm font-medium text-gray-700">Version {selectedVersion.number}</span>
                ...
                <div>
                  <span className="text-sm font-medium text-gray-700">
                    {compareVersion.id === 'current' ? 'Current Version' : `Version ${compareVersion.number}`}
                  </span>
                  <div className="text-xs text-gray-500">
              ...
              <ReactDiffViewer
                ...
                rightTitle={compareVersion.id === 'current' ? 'Current Version' : `Version ${compareVersion.number}`}
              />
            </div>
          ) : (
            <div>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-medium text-gray-900">
                  {selectedVersion.id === 'current' ? 'Current Version' : `Version ${selectedVersion.number}`}
                </h3>
              ...
              <div className="bg-gray-50 p-4 rounded-md mb-4">
                <div className="text-sm text-gray-500 flex items-center gap-1 mb-2">
                  <Clock size={14} />
                  {format(new Date(selectedVersion.createdAt), 'MMMM d, yyyy h:mm a')}
                </div>
