...
const Dashboard: React.FC = () => {
  ...
  const pendingApproval = documents.filter(doc => doc.status === 'pending_approval');
  const needsAttention = documents.filter(doc => doc.status === 'needs_attention');
          ...
          {documents.slice(0, 5).map((doc, index) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">{doc.lastModifiedAt}</p>
              </div>
            </div>
          ))}
        </div>
