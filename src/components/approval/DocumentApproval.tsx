...
import { CheckCircle, XCircle, AlertTriangle, Clock, UserCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
const DocumentApproval: React.FC<DocumentApprovalProps> = ({ document }) => {
  ...
  const handleApprove = () => {
    ...
    const currentStepIndex = document.approvalMatrix.findIndex(
      step => step.approvers.some(approver => approver.id === currentUser.id && !approver.approved && !approver.rejected)
    );
    ...
    const approverIndex = currentStep.approvers.findIndex(approver => approver.id === currentUser.id);
    ...
    const isStepCompleted = currentStep.approvers.every(approver => approver.approved || approver.rejected);
    if (isStepCompleted) {
      currentStep.completed = true;
      currentStep.completedAt = new Date().toISOString();
      
      // If all approvers in this step approved, move to next step
      const allApproved = currentStep.approvers.every(approver => approver.approved);
  ...
  const isUserCurrentApprover = () => {
    return document.approvalMatrix.some(
      step => step.active && !step.completed && step.approvers.some(
        approver => approver.id === currentUser.id && !approver.approved && !approver.rejected
      )
    );
  };
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="mt-1 flex items-center">
              <div className="w-40 bg-gray-200 rounded-full h-2.5 mr-2">
                <div 
                  ...
                  style={{ 
                    width: `${document.approvalMatrix.filter(step => step.completed).length / document.approvalMatrix.length * 100}%` 
                  }}
                ></div>
              </div>
              <span className="text-sm text-gray-500">
                {document.approvalMatrix.filter(step => step.completed).length}/{document.approvalMatrix.length} steps
              </span>
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        >
          ...
          <button className="text-gray-500">
            {showApprovalMatrix ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </button>
        </div>
                ...
                return (
                  <div 
                    ...
                    className={`border rounded-md overflow-hidden ${
                      step.active && !step.completed ? 'border-yellow-300' : 'border-gray-200'
                    }`}
                  >
                    <div className={`p-3 flex justify-between items-center ${
                      step.active && !step.completed ? 'bg-yellow-50' : 'bg-gray-50'
                    }`}>
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {step.type === 'single' ? 'Single Approver' : 
                             step.type === 'all' ? 'All Must Approve' : 
                             'Any Can Approve'}
                          </p>
                        </div>
                      </div>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getStepStatusClass(stepStatus)}`}>
                      ...
                      {step.approvers.map((approver, approverIndex) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            {(approver.approved || approver.rejected) && approver.comment && (
      ...
      {isUserCurrentApprover() && (
        <div className="p-6 border-t border-gray-200">
          <h3 className="text-sm font-medium text-gray-900 mb-4">Your Approval Action</h3>
