...
const DocumentCard: React.FC<DocumentCardProps> = ({ document, onClick }) => {
  ...
  return (
    <div 
      className="card transition-transform hover:shadow-md hover:scale-[1.02] cursor-pointer overflow-hidden h-full flex flex-col"
      onClick={onClick}
    >
      <div className="p-4 flex-grow">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-1 line-clamp-2">{document.title}</h3>
        <p className="text-sm text-gray-500 mb-3">
          {document.type} • {document.department}
        </p>
        <p className="text-sm text-gray-700 line-clamp-2 mb-4">{document.description}</p>
      </div>
